...
class PackageLoader {
	...
	const string PHPKG_MARKER = self::PHPKG_PREFIX . "GENERATED BY Userland Packages for PHP — DO NOT EDIT!!!\n";
	const string PHPKG_HEADER = self::PHPKG_MARKER . self::PHPKG_MANIFEST_PREFIX;
	...
	public function generatePhpkgPackage( string $pkgPath ): string {
		...
		$manifest = self::PHPKG_HEADER . $manifest;
		$checksummer->prepend($manifest);
		$package = sprintf( "%s\n%s",
			$manifest,
			implode( '', $fileContents ),
		);;
		return sprintf( "%s%s%s\n", $package, self::PHPKG_CHECKSUM_PREFIX, $checksummer->getSum() );
	}
	...
	private function parseAndLoadPhpkg( string $phpkg, Options $options ): string {
		...
		if ( ! str_starts_with( self::PHPKG_HEADER, $openTag ) ) {
			$this->parseError( $line, "unable to open tag; expected '%s', got '%s' instead for %s.",
				self::PHPKG_HEADER,
				substr( $openTag, strlen( self::PHPKG_HEADER ) ),
				$pkgFilepath
			);
		}
		unset( $openTag );
		...
		$checksummer = new Checksum(rtrim(self::PHPKG_MARKER.$manifest));
		...
		foreach ( $decodedManifest->files as $index => $filename ) {
			...
			if ( ! str_starts_with( $fileHeader, self::PHPKG_FILE_PREFIX ) ) {
				$this->parseError( $line, "unable to parse file header; expected '%s', got '%s' instead for %s of %s.",
					self::PHPKG_FILE_PREFIX,
					substr( $fileHeader, strlen( self::PHPKG_FILE_PREFIX ) ),
					$filename, $pkgFilepath
				);
			}
			$file = json_decode( substr( $fileHeader, strlen( self::PHPKG_FILE_PREFIX ) ) );
			if ( is_null( $file ) ) {
		...
		$checksum = fread( $pkgHandle, strlen(self::PHPKG_CHECKSUM_PREFIX)+40 );
		if ($checksum===false){
			throw new \Exception("Unable to read from {$pkgFilepath}.");
		}
		$checksum = substr($checksum,strlen(self::PHPKG_CHECKSUM_PREFIX));
		if ( $checksum != $checksummer->getSum() ) {
			$this->parseError( $line, "mismatch in checksum of %s.", $pkgFilepath );
		}
		return $pkgFilepath;
	}
